...
explore_LDAs_JH <- function(ldas,
  ...
  if(bigrams == TRUE){
    ...
    if (requireNamespace("ParseR", character.only = TRUE, quietly = T) == FALSE) {
...
.top_beta <- function(lda, top_n = 10, nrow = 2) {
  ...
  facetted <- top_terms %>%
    ...
    ggplot2::geom_segment(ggplot2::aes(x = term, xend = term,
                              y = 0, yend = beta),
                          show.legend = FALSE
    ) +
...
# Define function for making bigram within each topic
